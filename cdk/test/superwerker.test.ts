...
describe('parameters', () => {
  ...
  // Ignore parameters that are not used in the superwerker template
  for (const key in expectedParameters) {
    if (key.startsWith('QS')) delete expectedParameters[key]; // Quickstart
    if (key.startsWith('IncludeBudget')) delete expectedParameters[key];
    if (key.startsWith('IncludeGuardDuty')) delete expectedParameters[key];
    if (key.startsWith('IncludeSecurityHub')) delete expectedParameters[key];
    if (key.startsWith('IncludeBackup')) delete expectedParameters[key];
    if (key.startsWith('IncludeServiceControlPolicies')) delete expectedParameters[key];
  }
...
// for now TEST not needed since rewrite does not include all previous cloudformation Stacks
