import { CfnStack, Stack, StackProps } from 'aws-cdk-lib';
...
export class SuperwerkerStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: SuperwerkerStackProps) {
    ...
    // Prepare
    const prepareStack = new PrepareStack(this, 'PrepareAccount', {
      description: 'Prepares the AWS account for Superwerker.',
    });
    (prepareStack.node.defaultChild as CfnStack).overrideLogicalId('PrepareAccount');
    ...
    const controlTowerStack = new ControlTowerStack(this, 'ControlTower', {
      parameters: {
        AuditAWSAccountEmail: 'mail1@standalone-04.superwerker.aws.ffm.kreuzwerker.de',
        LogArchiveAWSAccountEmail: 'mail2@standalone-04.superwerker.aws.ffm.kreuzwerker.de',
      },
