...
## Disclaimer if you updated your Landingzone manually
...
if [ -z "$SUPERWERKER_STACK" ]; then
    ...
    if [ "$SUPERWERKER_VERSION" != "0.17.2" ]; then
        ...
        aws ssm put-parameter --type String --overwrite --name "/superwerker/controltower/regions" --value "${CONTROL_TOWER_REGIONS}"
        ...
        export CONTROL_TOWER_KMS_KEY=$(echo $LZ_MANIFEST | jq -r '.landingZone.manifest.centralizedLogging.configurations.kmsKeyArn')
        if [ "$CONTROL_TOWER_KMS_KEY" != "null" ]; then
            aws ssm put-parameter --type String --overwrite --name "/superwerker/controltower/kms_key" --value "${CONTROL_TOWER_KMS_KEY}"
        fi
